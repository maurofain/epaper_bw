...
19. implementiamo un programma python a caratteri che permetta di testare il procollo definito in docs/RS232_protocol.md. all'avvio dove chiedere la porta di comunicazione del PC a cui è connessa la scheda (default /dev/ttyACM0) permettendo di editarla, se la porta è disponibili si passa ll menu successivo altrimenti ripete la richiesta della porta. Il menu deve permettere di inviare tutti i comandi del protocollo con dei comandi a 1 lettera per schegliere le opzioni dei messaggi tipo 0,1, e 2. deveonoesserci 2 voci di menu : una per i testi con scelta automatica del font (len 0-0xFE) e uno per test con scelta font e posizione con il byte len = 0xFF . per il secondo tipo va chiesto il font (con un numero da 1 a 6) e proposta la posizione centrale (100,100) su cui centrare il testo. Quindi va richiesto di imputare il test